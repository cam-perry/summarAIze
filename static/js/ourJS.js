...
Chart = function() {
  ...
  return self = {
    create: function(el) {
			...
			x = d3.scaleBand().rangeRound([margin.left, innerWidth]).padding(0.1);
        ...
        .text(yVal);
			...
			bars.data(data)
					...
					.attr("height", function(d) { return innerHeight - y(d[yVal]); });
      ...
      return self;
		...
		redraw: function() {
			...
			//Update Axes
			x.rangeRound([margin.left, innerWidth])
				.domain(data.map(function(d) { return d[xVal]; }));
			y.rangeRound([innerHeight, margin.top])
